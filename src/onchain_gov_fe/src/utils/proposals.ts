...
export const isLicenseReleaseProposal = (p: Proposal) => {
  ...
  const hasArgs = p.votes.length === 2 && p.votes[1].args?.fields?.length === 3

  return correctVoteTypes && hasArgs
}

export const parseLicenseReleaseArgs = (proposal: Proposal) => {
  const licenseArgs = proposal.votes[1].args
  ...
  const approveWeight = 50 ?? proposal.votes[1].weight
  const revokeWeight = 20 ?? proposal.votes[0].weight
