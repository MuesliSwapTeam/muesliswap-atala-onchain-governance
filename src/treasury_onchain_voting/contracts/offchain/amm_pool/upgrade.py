...
from contracts.onchain.amm_pool import amm_pool, pool_nft
from contracts.onchain.treasury import (
    treasurer_nft,
    treasurer,
    value_store,
)
from contracts.utils.from_script_context import (
    from_value,
    from_output_datum,
    from_address,
)
...
def main(
    ...
    (
        ...
        _,
        amm_pool_address,
    ) = get_contract(module_name(amm_pool), True)
    (
        _,
        pool_nft_policy_id,
        _,
    ) = get_contract(module_name(pool_nft), True)
    amm_pool_script = get_ref_utxo(amm_pool_script, context) or amm_pool_script
    ...
    # select pool
    pool_utxos = context.utxos(amm_pool_address)
    pool_utxo = None
    pool_datum = None
    for u in pool_utxos:
        try:
            datum = amm_pool.PoolState.from_cbor(u.output.datum.cbor)
        except Exception:
            continue
    ...
    for u in tally_utxos:
        ...
        try:
            winning_proposal: amm_pool.PoolUpgradeParams = (
                amm_pool.PoolUpgradeParams.from_cbor(winning_proposal.to_cbor())
            )
    ...
    all_reference_utxos = sorted_utxos(
        ...
        + (
            [amm_pool_script]
            if isinstance(amm_pool_script, pycardano.TransactionOutput)
            else []
        )
    )
    tally_input_index = all_reference_utxos.index(tally_state_utxo)
    if isinstance(winning_proposal.new_pool_params, amm_pool.Nothing):
    ...
    new_pool_state = amm_pool.PoolState(
    ...
    new_pool_address = (
        ...
        if isinstance(winning_proposal.new_pool_address, amm_pool.Nothing)
